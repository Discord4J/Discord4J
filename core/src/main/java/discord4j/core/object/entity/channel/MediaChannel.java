...
import discord4j.core.spec.MediaChannelEditSpec;
import discord4j.core.spec.MediaThreadMessageCreateSpec;
import discord4j.core.spec.StartThreadInMediaChannelMono;
import discord4j.core.spec.StartThreadInMediaChannelSpec;
...
public final class MediaChannel extends BaseTopLevelGuildChannel implements CategorizableChannel {
     ...
     * @return A {@link Mono} that, upon completion, emits the updated {@link MediaChannel} object
     */
    public Mono<MediaChannel> edit(MediaChannelEditSpec spec) {
     ...
     */
    public Mono<ThreadChannel> startThread(StartThreadInMediaChannelSpec request) {
        return this.getClient().getRestClient().getChannelService().startThreadInMediaChannel(getId().asLong(), request.asRequest())
     ...
     * can be set via the {@code withXxx} methods of the returned {@link StartThreadInMediaChannelMono}.
     *
     * @param name The name of the thread
     * @param messageCreateSpec The message to start the thread with
     * @return A {@link StartThreadInMediaChannelMono} where, upon successful completion, emits the created
     * {@link ThreadChannel}. If an error is received, it is emitted through the {@code Mono}.
     */
    public StartThreadInMediaChannelMono startThread(String name, MediaThreadMessageCreateSpec messageCreateSpec) {
