...
interface StageChannelJoinSpecGenerator extends Spec<Function<StageChannel, Mono<VoiceConnection>>> {
    ...
    @Override
    default Function<StageChannel, Mono<VoiceConnection>> asRequest() {
        return voiceChannel -> {
            final Logger log = Loggers.getLogger(StageChannelJoinSpec.class);
...
abstract class StageChannelJoinMonoGenerator extends Mono<VoiceConnection> implements StageChannelJoinSpecGenerator {

    abstract StageChannel channel();

    @Override
    public void subscribe(CoreSubscriber<? super VoiceConnection> actual) {
        channel().join(StageChannelJoinSpec.copyOf(this)).subscribe(actual);
    }
