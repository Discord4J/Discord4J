...
/**
 * Provides Reactor Netty resources like an {@link reactor.netty.http.client.HttpClient} that can be customized and
 * reused across the application.
 * <p>
 * Allow a user to externally manage the connection pool through a custom
 * {@link reactor.netty.resources.ConnectionProvider}, and custom event loop threads using a
 * {@link reactor.netty.resources.LoopResources}.
 */
public class ReactorResourceProvider {
    ...
    /**
     * Create with a default {@link reactor.netty.http.client.HttpClient} instance.
     */
    public ReactorResourceProvider() {
     ...
     * application shutdown, through {@link reactor.netty.resources.ConnectionProvider#dispose()} and
     * {@link reactor.netty.resources.LoopResources#dispose()}.
     *
     ...
     */
    public ReactorResourceProvider(ConnectionProvider provider, LoopResources resources,
            Function<HttpClient, HttpClient> mapper) {
        this.httpClient = initializer.andThen(mapper).apply(initHttpClient(provider, resources));
    }

    /**
     * Create with a pre-configured {@link reactor.netty.http.client.HttpClient} instance.
     *
     * @param httpClient the underlying HttpClient to use
     */
    public ReactorResourceProvider(HttpClient httpClient) {
    ...
    /**
     * Get the {@link reactor.netty.http.client.HttpClient} configured by this provider.
     *
